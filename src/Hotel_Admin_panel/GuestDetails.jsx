...
const GuestDetails = () => {
  ...
  useEffect(() => {
    const fetchGuestsForCurrentHotel = async () => {
      try {
        const user = auth.currentUser; // ✅ logged-in hotel user
        if (!user) {
        ...
        guestSnap.forEach((docSnap) => {
          ...
          const paymentProof = Array.isArray(data["Payment Proof"])
            ? data["Payment Proof"]
            : [];
          const proofUrls = paymentProof
            .map((item) => item.url)
            .filter(Boolean);
...
<div className="mb-3">
  <div className="text-muted small">Payment Status</div>
  <div>
    {guest["Payment Status"] || "N/A"}{" "}
    {guest["Payment Status"]?.toLowerCase().trim() === "paid" && (
      <i className="bi bi-patch-check-fill" style={{ color: "green", marginLeft: "5px" }}></i>
    )}
  </div>
</div>
